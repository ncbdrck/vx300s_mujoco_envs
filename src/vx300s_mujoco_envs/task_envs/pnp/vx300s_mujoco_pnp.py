...
register(
    id='VX300SMujocoPnpSim-v0',
    entry_point='vx300s_mujoco_envs.task_envs.pnp.vx300s_mujoco_pnp:VX300SMujocoPnpEnv',
    max_episode_steps=100,
)


class VX300SMujocoPnpEnv(vx300s_mujoco_robot.VX300SMujocoRobotEnv):
    """
    Pick-and-place task for the VX300S robot on the MuJoCo backend.

    The arm must grasp a cube resting on the table and move it to a 3D goal above the surface
    (so the task requires both a pick and a lift/place). Built on the same robot env and real-time
    loop as the reach/push tasks, but additionally drives the gripper: the action carries one extra
    scalar (the left-finger position; the right finger mirrors it), and a gripper controller is
    spawned. The cube is a free-joint body in the MJCF scene, repositioned each episode via the
    mujoco_ros set_body_state service. The goal is shown as an RViz marker.

    The task is done when the cube is within the success tolerance of the 3D goal.

    Action Space - Continuous (6 arm-joint deltas + 1 gripper scalar).
    Observation  - Continuous (EE pos, cube pos, gripper opening, unit vector EE->cube,
                   unit vector cube->goal, distance cube->goal, joint values, previous action,
                   joint velocities).
    ...
    def __init__(self, launch_mujoco: bool = True, new_roscore: bool = True, roscore_port: str = None,
        ...
        if ros_port is not None:
            self.node_name = "VX300SMujocoPnpEnvSim" + "_" + ros_port
        else:
            self.node_name = "VX300SMujocoPnpEnvSim"
        ...
        high = np.concatenate(
            [observations_high_ee_pos, observations_high_cube_pos, observations_high_gripper,
             observations_high_vec_ee_cube, observations_high_vec_cube_goal, observations_high_dist,
        ...
        low = np.concatenate(
            [observations_low_ee_pos, observations_low_cube_pos, observations_low_gripper,
             observations_low_vec_ee_cube, observations_low_vec_cube_goal, observations_low_dist,
             observations_low_joint_values, observations_low_prev_action, observations_low_joint_vel, ])

        self.observation_space = spaces.Box(low=low, high=high, dtype=np.float32)

        # ---- goal sampling space (3D, above the table -> requires grasp + lift)
        ...
        # robot/controller stack already brought up by the launch file. Pass the gripper-enabled
        # controller + joint sets so the gripper controller is spawned and the finger transmissions
        # are kept in robot_description.
    ...
    def _set_init_params(self, options: Optional[Dict[str, Any]] = None):
        """
        Reset to the home pose (gripper open), reposition the cube, and sample a new 3D goal.
        """
        ...
        # move the robot to the home pose and open the gripper
        ...
        # sample a 3D goal above the table for the cube to be moved to
        if self.random_goal:
    ...
    def execute_action(self, action):
        """
        Apply the action: the first 6 entries are arm-joint deltas, the last is the gripper scalar.
        Add the (scaled) arm deltas to the current joints, clip, run the workspace + per-link
        safety checks, then publish the arm trajectory and the gripper command.
        """
        action = np.asarray(action, dtype=np.float32)
        arm_action = action[:6]
        gripper_value = action[6] if action.shape[0] > 6 else self.gripper_cmd

        # delta action on top of current joints (arm only)
        ...
        self.action_not_in_limits = np.any(arm_target <= (min_joint_values + 0.0001)) or np.any(
            arm_target >= (max_joint_values - 0.0001))
    ...
    def get_gripper_opening(self):
        """
        Return the current left-finger position from the joint state if available, else the last
        commanded value.
        """
    ...
    def sample_observation(self):
        """
        Build the observation vector: EE pos, cube pos, gripper opening, unit vector EE->cube,
        unit vector cube->goal, distance cube->goal, joint positions, previous action,
        joint velocities.
        """
        ...
        vec_cube_goal = self._safe_unit_vector(self.pnp_goal - self.cube_pos)
        euclidean_distance_cube_goal = scipy.spatial.distance.euclidean(self.cube_pos, self.pnp_goal)
        ...
        obs = np.concatenate((self.ee_pos, self.cube_pos, gripper_opening, vec_ee_cube, vec_cube_goal,
    ...
    def calculate_reward(self):
        """
        Sparse: +1 when the cube is at the 3D goal, -1 otherwise.
        Dense (simple): negative (distance EE->cube + distance cube->goal), so the agent is
        rewarded for both reaching the cube and moving it toward the goal.
        """
        reward = 0.0
    ...
    def check_if_done(self):
        """
        Done when the cube is within the success tolerance of the 3D goal.
        """
        done_pnp = self.check_if_reach_done(self.cube_pos, self.pnp_goal)
        if done_pnp:
            done = True
            self.current_action = None
            self.init_done = False
    ...
    def check_if_reach_done(self, achieved_goal, desired_goal):
        # Pick-and-place success is a full 3D distance (the goal is above the table).
    ...
    def check_action_within_workspace(self, action):
        """
        True if the FK of the arm action lands inside the workspace box.
        """
    ...
    def _get_params(self):
        """
        Load task parameters from the parameter server (set by vx300s_pnp_task_config.yaml).
        """
        ...
        # Cube observation bounds (wide) and the cube spawn region (narrow) are separate: a grasped
        # cube travels into the 3D goal region, so the observation box must be wider than the spawn box.
    ...
    def _launch_mujoco(self, launch_roscore=True, port=None, paused=False, use_sim_time=True,
        ...
        """
        Launch a MuJoCo server for the pick-and-place scene.
        """
