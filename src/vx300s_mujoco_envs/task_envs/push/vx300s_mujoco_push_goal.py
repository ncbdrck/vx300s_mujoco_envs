...
register(
    id='VX300SMujocoPushGoalSim-v0',
    entry_point='vx300s_mujoco_envs.task_envs.push.vx300s_mujoco_push_goal:VX300SMujocoPushGoalEnv',
    max_episode_steps=100,
)


class VX300SMujocoPushGoalEnv(vx300s_mujoco_robot_goal.VX300SMujocoRobotGoalEnv):
    """
    Goal-conditioned (HER-ready) push task for the VX300S robot on the MuJoCo backend.

    Same task as the standard push env, but exposes the gymnasium-robotics GoalEnv contract: the
    ...
    goals. The achieved goal is the cube position; the desired goal is the sampled push target on
    the table.

    Action Space - Continuous (6 arm-joint deltas).
    Observation  - Dict:
        observation   : EE pos, cube pos, joint values, previous action, joint velocities.
        achieved_goal : cube position (3,).
        desired_goal  : push target on the table (3,).
    """
    ...
    def __init__(self, launch_mujoco: bool = True, new_roscore: bool = True, roscore_port: str = None,
        ...
        if ros_port is not None:
            self.node_name = "VX300SMujocoPushGoalEnvSim" + "_" + ros_port
        else:
            self.node_name = "VX300SMujocoPushGoalEnvSim"
        ...
        # load task parameters onto the parameter server (shared with the standard push task)
        ...
        # ---- observation bounds (proprioceptive + cube pos; no goal-derived features)
        ...
        obs_high = np.concatenate(
            [observations_high_ee_pos, observations_high_cube_pos, observations_high_joint_values,
             observations_high_prev_action, observations_high_joint_vel, ])
        obs_low = np.concatenate(
            [observations_low_ee_pos, observations_low_cube_pos, observations_low_joint_values,
             observations_low_prev_action, observations_low_joint_vel, ])

        # ---- goal spaces (achieved = cube pos, desired = push target); achieved ranges over the
        # cube observation box, desired over the goal region.
        ...
        self.push_goal = np.zeros(3, dtype=np.float32)
    ...
    def _get_desired_goal(self):
        ...
        return np.asarray(self.push_goal, dtype=np.float32)

    def compute_reward(self, achieved_goal, desired_goal, info):
        """
        Sparse HER reward: 0 when the cube is within the success tolerance of the goal (measured in
        the table plane), else -1. Batch-safe: SB3's HER calls this with (N, 3) arrays as well as a
        single (3,) goal, so the distance is computed along the last axis and the result keeps that
        shape.
        """
        achieved = np.asarray(achieved_goal, dtype=np.float32)
        desired = np.asarray(desired_goal, dtype=np.float32)
        # in-plane distance (x, y); the cube stays on the table for push
        distance = np.linalg.norm(achieved[..., :2] - desired[..., :2], axis=-1)
        return -(distance > self.reach_tolerance).astype(np.float32)

    def compute_terminated(self, achieved_goal, desired_goal, info):
        a = np.asarray(achieved_goal)
        d = np.asarray(desired_goal)
        distance = np.linalg.norm(a[..., :2] - d[..., :2], axis=-1)
    ...
    def environment_loop(self, event):
        if self.init_done:
            ...
            self.desired_r = np.asarray(self.push_goal, dtype=np.float32)
    ...
    def sample_observation(self):
        ...
        obs = np.concatenate((self.ee_pos, self.cube_pos,
