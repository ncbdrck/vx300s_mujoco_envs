...
class VX300SMujocoReacherEnv(vx300s_mujoco_robot.VX300SMujocoRobotEnv):
    """
    Simple Reach task for the VX300S robot on the MuJoCo backend.

    Mirrors the Gazebo VX300S reach task (same observation/action layout, reward architecture
    and real-time loop from the UniROS paper), but runs against the MuJoCo server and uses the
    ros_control trajectory interface directly (no MoveIt). The reach goal is an RViz marker, so
    the task needs no runtime object spawning.

    The task is done when the end-effector reaches the goal within the reach tolerance.

    Action Space - Continuous (6 arm-joint deltas).
    Observation  - Continuous (EE pos, unit vector to goal, distance, joint values,
                   previous action, joint velocities).

    Real-time vs normal MDP mode is selected by ``realtime_mode``:
      * realtime_mode=True (default): the UniROS paper real-time loop. Physics is never paused;
        a rospy.Timer at ``environment_loop_rate`` refreshes obs/reward/done, and ``step()``
        reads the latest cached values. This matches the real robot, so a policy transfers.
      * realtime_mode=False: the standard MDP loop. Physics is paused around ``_set_action``,
        the action runs synchronously, the agent waits ``action_cycle_time`` for the trajectory,
        then a fresh obs/reward/done is sampled. For clean algorithm benchmarking.
    """
    ...
    def __init__(self, launch_mujoco: bool = True, new_roscore: bool = True, roscore_port: str = None,
        ...
        if ros_port is not None:
            self.node_name = "VX300SMujocoReacherEnvSim" + "_" + ros_port
        else:
            self.node_name = "VX300SMujocoReacherEnvSim"
        ...
        observations_high_ee_pos_range = np.array([self.position_ee_max["x"], self.position_ee_max["y"],
                                                   self.position_ee_max["z"]])
        observations_low_ee_pos_range = np.array([self.position_ee_min["x"], self.position_ee_min["y"],
        ...
        if environment_loop_rate is not None:
            ...
            # Real-time mode: drive the env loop with a rospy.Timer (paper §7). Normal mode
    ...
    def _set_init_params(self, options: Optional[Dict[str, Any]] = None):
        """
        Reset to the home pose and sample a new reach goal.
        """
    ...
    def environment_loop(self, event):
        """
        Real-time RL loop (paper §7). Periodically refreshes obs/reward/done and re-applies the
        latest action (action repeats) so the agent never waits on sensor/actuator processing.
        """
        if self.init_done:
    ...
    def sample_observation(self):
        """
        Build the observation vector: EE pos, unit vector to goal, Euclidean distance, joint
        positions, previous action, joint velocities.
        """
        ...
        linear_dist_ee_goal = current_goal - self.ee_pos
        vec_ee_goal = self._safe_unit_vector(linear_dist_ee_goal)
        euclidean_distance_ee_goal = scipy.spatial.distance.euclidean(self.ee_pos, current_goal)
    ...
    def calculate_reward(self):
        """
        Sparse: +1 on reach, -1 otherwise. Dense (simple): negative Euclidean distance to goal.
        """
        reward = 0.0
    ...
    def check_if_done(self):
        """
        Done when the EE is within the reach tolerance of the goal.
        """
    ...
    def _get_params(self):
        """
        Load task parameters from the parameter server (set by vx300s_reach_task_config.yaml).
        """
        ...
        # order /vx300s/joint_states publishes). The reused Gazebo config carries a 9-element,
        # differently-ordered layout (it includes passive gripper/finger joints); deriving the
        # bounds from the URDF here keeps the observation space consistent with what the MuJoCo
        # backend actually publishes for this arm-only reach task.
    ...
    def _launch_mujoco(self, launch_roscore=True, port=None, paused=False, use_sim_time=True,
        ...
        """
        Launch a MuJoCo server for the reach scene.
        """
